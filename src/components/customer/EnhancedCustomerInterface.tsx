...
const EnhancedCustomerInterface = ({
  onSwitchRole
}: EnhancedCustomerInterfaceProps) => {
  const {
    modals,
  ...
  const filteredProducts = filterProducts(mockProducts, state.searchQuery, state.selectedFilter, state.filters);
  ...
  return <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-green-100">
      ...
      <HeroSection searchQuery={state.searchQuery} onSearchChange={stateHandlers.setSearchQuery} />

      <div className="container mx-auto px-4">
        <PromoBanners />
        
        <OdooControlPanel onSyncOpen={() => modalHandlers.setIsOdooSyncOpen(true)} />
      </div>

      <FiltersSection selectedFilter={state.selectedFilter} onFilterChange={stateHandlers.setSelectedFilter} onFiltersOpen={() => modalHandlers.setIsFiltersOpen(true)} onRecipesOpen={() => modalHandlers.setIsRecipesOpen(true)} onSchedulerOpen={() => modalHandlers.setIsSchedulerOpen(true)} />

      <FeatureButtons onLoyaltyOpen={() => modalHandlers.setIsLoyaltyOpen(true)} onBulkOrderOpen={() => modalHandlers.setIsBulkOrderOpen(true)} onSubscriptionOpen={() => modalHandlers.setIsSubscriptionOpen(true)} onQuickReorderOpen={() => modalHandlers.setIsQuickReorderOpen(true)} onReferralOpen={() => modalHandlers.setIsReferralOpen(true)} onSocialSharingOpen={() => modalHandlers.setIsSocialSharingOpen(true)} />

      <ProductsGrid products={filteredProducts} wishlistItems={state.wishlistItems} onAddToCart={handleAddToCart} onToggleWishlist={handleToggleWishlist} onOpenReviews={openReviews} onOpenNutrition={openNutritionInfo} />

      <Footer />

      <ModalsContainer isAuthOpen={modals.isAuthOpen} isCartOpen={modals.isCartOpen} isProfileOpen={modals.isProfileOpen} isTrackingOpen={modals.isTrackingOpen} isWishlistOpen={modals.isWishlistOpen} isReviewsOpen={modals.isReviewsOpen} isNotificationOpen={modals.isNotificationOpen} isSchedulerOpen={modals.isSchedulerOpen} isRecipesOpen={modals.isRecipesOpen} isFiltersOpen={modals.isFiltersOpen} selectedProductForReview={state.selectedProductForReview} cartItems={state.cartItems} filters={state.filters} onAuthClose={() => modalHandlers.setIsAuthOpen(false)} onCartClose={() => modalHandlers.setIsCartOpen(false)} onProfileClose={() => modalHandlers.setIsProfileOpen(false)} onTrackingClose={() => modalHandlers.setIsTrackingOpen(false)} onWishlistClose={() => modalHandlers.setIsWishlistOpen(false)} onReviewsClose={() => modalHandlers.setIsReviewsOpen(false)} onNotificationClose={() => modalHandlers.setIsNotificationOpen(false)} onSchedulerClose={() => modalHandlers.setIsSchedulerOpen(false)} onRecipesClose={() => modalHandlers.setIsRecipesOpen(false)} onFiltersClose={() => modalHandlers.setIsFiltersOpen(false)} onSetCartItems={stateHandlers.setCartItems} onAddToCart={handleAddToCart} onScheduleDelivery={handleScheduleDelivery} onApplyFilters={handleApplyFilters} onLogin={() => stateHandlers.setIsLoggedIn(true)} />
